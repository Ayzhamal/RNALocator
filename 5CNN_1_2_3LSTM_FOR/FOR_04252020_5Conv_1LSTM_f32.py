...
for window in range(10, 550, 50):
    ...
    plt.legend()
    plt.show()
    fig_loss.savefig(plots_path+'plot_loss_b=256_l='+str(layers)+'CNN_LSTM_f='+str(filters)+'_w='+str(window)+'.png')
    ...
    plt.legend()
    plt.show()
    fig_mae.savefig(plots_path+'plot_mae_b=256_l='+str(layers)+'CNN_LSTM_f='+str(filters)+'_w='+str(window)+'.png')
...
print("\nDataFrame for a model with "+str(layers)+" Conv layers + 1LSTM batch_size=256 with "+str(filters)+" filters:\n", df)
df.to_csv(plots_path+"Dataframe_batch=256_l="+str(layers)+"Conv_1LSTM_f="+str(filters)+".csv")
