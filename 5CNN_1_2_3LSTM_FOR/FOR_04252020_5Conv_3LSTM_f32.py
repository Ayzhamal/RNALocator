...
for window in range(10, 550, 50):
    print("\nChecking "+str(layers)+" Conv_3LSTM layers with batch_size=256, f="+str(filters)+" and w="+str(window)+":\n--------------")
    model=Sequential()
    ...
    filepath=plots_path+"model_"+str(layers)+"CNN_3LSTM_batch=256_f="+str(filters)+"_w="+str(window)+".best.hdf5"
    ...
    plt.legend()
    plt.show()
    fig_loss.savefig(plots_path+'plot_loss_b=256_l='+str(layers)+'CNN_3LSTM_f='+str(filters)+'_w='+str(window)+'.png')
    ...
    plt.legend()
    plt.show()
    fig_mae.savefig(plots_path+'plot_mae_b=256_l='+str(layers)+'CNN_3LSTM_f='+str(filters)+'_w='+str(window)+'.png')
...
print("\nDataFrame for a model with "+str(layers)+" Conv layers + 3LSTM batch_size=256 with "+str(filters)+" filters:\n", df)
df.to_csv(plots_path+"Dataframe_batch=256_l="+str(layers)+"Conv_3LSTM_f="+str(filters)+".csv")
