...
def create_conv(layer_sizes):
    ...
    model = Sequential()
    model.add(Conv1D(filters=layer_sizes[0], kernel_size=110, activation='relu', input_shape= (x_train.shape[1], 4)))
...
def evaluate(model, layer_sizes, batch_size=256, epochs=50):
    ...
    filepath=path+"model_data="+dataset+"_"+str(len(layer_sizes))+"_conv_layers_f=_"+str(layer_sizes[0])+".best.hdf5"
...
print("\nDataFrame for a model with grid Conv1D layers for "+dataset+" dataset:\n", df)
df.to_csv(path+"Dataframe_grid_conv_"+dataset+".csv")
